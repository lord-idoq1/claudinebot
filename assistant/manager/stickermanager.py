...
async def kang_cmd(ayra):
    sender = await ayra.get_sender()
    if not isinstance(sender, User):
        return
    if not ayra.is_reply:
        return await ayra.eor("`Reply to a sticker/photo..`", time=5)
    ...
    try:
        emoji = ayra.text.split(maxsplit=1)[1]
    except IndexError:
        emoji = None
    if reply.sticker:
        ...
        return await ayra.eor("`Reply to sticker or text to add it in your pack...`")
    if not emoji:
        emoji = "🏵"
    if dl:
        upl = await ayra.client.upload_file(dl)
        file = get_input_document(
            await ayra.client(UploadMediaRequest(InputPeerSelf(), upl))
        )
    get_ = udB.get_key("STICKERS") or {}
    type_ = "anim" if animated else "static"
    if not get_.get(ayra.sender_id) or not get_.get(ayra.sender_id, {}).get(type_):
        sn = f"{pre}_{ayra.sender_id}"
        ...
        try:
            ...
            sn = sn.replace(str(ayra.sender_id), f"{ayra.sender_id}_{ayra.id}")
        except StickersetInvalidError:
            pass
        try:
            pack = await ayra.client(
            ...
            return await ayra.eor(str(er))
        sn = pack.set.short_name
        if not get_.get(ayra.sender_id):
            get_.update({ayra.sender_id: {type_: [sn]}})
        else:
            get_[ayra.sender_id].update({type_: [sn]})
        udB.set_key("STICKERS", get_)
        return await ayra.reply(
            f"**Kanged Successfully!\nEmoji :** {emoji}\n**Link :** [Click Here](https://t.me/addstickers/{sn})"
        )
    name = get_[ayra.sender_id][type_][-1]
    try:
        await asst(GetSticker(InputStickerSetShortName(name), hash=0))
    except StickersetInvalidError:
        get_[ayra.sender_id][type_].remove(name)
    try:
        await asst(
        ...
        sn = f"{pre}{ayra.sender_id}_{ayra.id}"
        title = f"{get_display_name(sender)}'s Kang Pack"
        if animated:
            sn += "_anim"
            title += " (Animated)"
        ...
        try:
            ...
            return await ayra.eor(str(er))
        get_[ayra.sender_id][type_].append(pack.set.short_name)
        udB.set_key("STICKERS", get_)
        return await ayra.reply(
        ...
        LOGS.exception(er)
        return await ayra.reply(str(er))
    await ayra.reply(
        f"Stiker Berhasil Ditambahkan ke Paket\n**Link :** [Click Here](https://t.me/addstickers/{name})"
    )


@asst_cmd(pattern="listpack")
async def do_magic(ayra):
    ko = udB.get_key("STICKERS") or {}
    if not ko.get(ayra.sender_id):
        return await ayra.reply("No Sticker Pack Found!")
    al_ = []
    ul = ko[ayra.sender_id]
    for _ in ayra.keys():
        al_.extend(ul[_])
    msg = "• **Stiker Milik Anda!**\n\n"
    for _ in al_:
        try:
            pack = await ayra.client(GetSticker(InputStickerSetShortName(_), hash=0))
            msg += f"• [{pack.set.title}](https://t.me/addstickers/{_})\n"
        except StickerSetInvalidError:
            if ayra.get("anim") and _ in ul["anim"]:
                ul["anim"].remove(_)
            elif ayra.get("vid") and _ in ul["vid"]:
                ul["vid"].remove(_)
            else:
                ul["static"].remove(_)
            udB.set_key("STICKERS", ko)
    await ayra.reply(msg)
