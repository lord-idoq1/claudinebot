...
ultpic = "https://graph.org/file/02f9ca4617cec58377b9d.jpg"
...
async def twitter_search(event):
    ...
    if _ayra_cache.get("twitter"):
        _ayra_cache["twitter"].update({match: reso})
    else:
        _ayra_cache.update({"twitter": {match: reso}})
...
async def inline_tl(ayra):
    ...
    await ayra.answer(res[:50], switch_pm=mo, switch_pm_param="start")
