...
_buttons = {
    ...
    "chatbot": {
        "text": "Dari Fitur Ini Anda dapat mengobrol dengan seseorang lewat Assistant Bot.",
        "buttons": [
            [
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**✪ ᴀʏʀᴀ ꭙ ᴜꜱᴇʀʙᴏᴛ​ ✪**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET",
        buttons=[
            [
...
async def hndlrr(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"Kirim Simbol yang Anda inginkan Handler/Trigger untuk menggunakan bot\nHandler Saat Ini adalah [ `{HNDLR}` ]\n\n gunakan /cancel untuk membatalkan.",
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
        if themssg == "/cancel":
            ...
            await conv.send_message(
                "Ini tidak dapat digunakan sebagai handler",
                buttons=get_back_button("cbs_otvars"),
            )
        else:
            await setit(event, var, themssg)
...
async def hndlrr(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            "Kirim Simbol yang Anda inginkan Sudo Handler/Trigger untuk menggunakan bot\n\n gunakan /cancel untuk membatalkan."
        )
...
async def eddon(event):
    ...
    await event.edit(
        "Selesai! ADDONS telah diaktifkan!!\n\n Setelah Menyetel Semua Hal Lakukan Mulai Ulang",
...
async def pmset(event):
    ...
    await event.edit(
        f"SUDO MODE ~ Beberapa orang dapat menggunakan Bot Anda yang Anda pilih. Untuk mengetahui lebih banyak gunakan `{HNDLR}help sudo`",
        buttons=[
            BT,
            [Button.inline("« Bᴀᴄᴋ", data="cbs_otvars")],
        ],
...
async def inl_on(event):
    ...
    await setit(event, var, "True")
    await event.edit(
        "Selesai!! Jenis PMPermit telah diatur ke sebaris!",
...
async def inl_on(event):
    ...
    await setit(event, var, "False")
    await event.edit(
        "Selesai!! Jenis PMPermit telah diatur ke normal!",
        buttons=[[Button.inline("« Bᴀᴄᴋ", data="cbs_pmtype")]],
    )
...
async def name(event):
    m = range(1, 10)
    tayrad = [Button.inline(f"{x}", data=f"wrns_{x}") for x in m]
    lst = list(zip(tayrad[::3], tayrad[1::3], tayrad[2::3]))
