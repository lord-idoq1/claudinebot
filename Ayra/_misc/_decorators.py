...
def ayra_cmd(
    ...
    def decor(dec):
        async def wrapp(ayra):
            if not ayra.out:
                if owner_only:
                    return
                if ayra.sender_id not in owner_and_sudos():
                    return
                if ayra.sender_id in _ignore_eval:
                    return await eod(
                        ayra,
                        get_string("py_d1"),
                    )
                if fullsudo and ayra.sender_id not in SUDO_M.fullsudos:
                    return await eod(ayra, get_string("py_d2"), time=15)
            chat = ayra.chat
            if hasattr(chat, "title"):
                if (
                    "#noub" in chat.title.lower()
                    and not (chat.admin_rights or chat.creator)
                    and not (ayra.sender_id in DEVLIST)
                ):
                    return
            if admins_only:
                if ayra.is_private:
                    return await eod(ayra, get_string("py_d3"))
                if not (chat.admin_rights or chat.creator):
                    return await eod(ayra, get_string("py_d5"))
            ...
            if groups_only and ayra.is_private:
                return await eod(ayra, get_string("py_d5"))
            try:
                await dec(ayra)
                ...
                return await eod(ayra, "`Inline Locked In This Chat.`")
            except (ChatSendMediaForbiddenError, ChatSendStickersForbiddenError):
                return await eod(ayra, get_string("py_d8"))
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(ayra, get_string("py_d6"))
                ...
                ftext += "\n**Date:** `" + date
                ftext += "`\n**Group:** `" + str(ayra.chat_id) + "` " + str(naam)
                ftext += "\n**Sender ID:** `" + str(ayra.sender_id)
                ftext += "`\n**Replied:** `" + str(ayra.is_reply)
                ftext += "`\n\n**Event Trigger:**`\n"
                ftext += str(ayra.text)
                ftext += "`\n\n**Traceback info:**`\n"
        ...
        if manager and MANAGER:
            ...
            async def manager_cmd(ayra):
                if not allow_all and not (await admin_check(ayra, require=require)):
                    return
                if not allow_pm and ayra.is_private:
                    return
                try:
                    await dec(ayra)
                except Exception as er:
                    if chat := udB.get_key("MANAGER_LOG"):
                        text = f"**#MANAGER_LOG\n\nChat:** `{get_display_name(ayra.chat)}` `{ayra.chat_id}`"
                        text += f"\n**Replied :** `{ayra.is_reply}`\n**Command :** {ayra.text}\n\n**Error :** `{er}`"
                        try:
                            ...
                            LOGS.exception(er)
                    LOGS.info(f"• MANAGER [{ayra.chat_id}]:")
                    LOGS.exception(er)
