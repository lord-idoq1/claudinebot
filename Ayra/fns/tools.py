...
async def get_chatbot_reply(message):
    ...
    chatbot_base = "https://kukiapi.xyz/api/apikey=AYRAUSERBOT/Ayra/{}/message={}"
...
def check_filename(filroid):
    if os.path.exists(filroid):
        no = 1
        while True:
            ayra = "{0}_{2}{1}".format(*os.path.splitext(filroid) + (no,))
            if os.path.exists(ayra):
                no += 1
            else:
                return ayra
    return filroid
