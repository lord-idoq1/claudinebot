...
from Ayra.fns.tools import four_point_transform
...
async def pdfseimg(event):
    ...
    if not (ok and (ok.document and (ok.document.mime_type == "application/pdf"))):
        await event.eor("`Balas pdf yang ingin Anda unduh..`")
        return
    ...
    result = await downloader(
        f"pdf/{filename}", file, xx, k, f"Mengunduh {filename}..."
    )
    ...
    if not msg:
        ...
        for num in range(pdf.numPages):
            ...
            fil = os.path.join(f"pdf/ayra{num + 1}.png")
            ok.append(fil)
        ...
        for o in range(ok):
            ...
            with open(os.path.join("ayra.png"), "wb") as f:
                pw.write(f)
            await event.reply(
                file="ayra.png",
            )
        ...
        os.remove(pdfp)
        try:
            await event.reply(file="ayra.png")
        except PhotoSaveFileInvalidError:
            await event.reply(file="ayra.png", force_document=True)
        os.remove("ayra.png")
...
async def pdfsetxt(event):
    ...
    result = await downloader(filename, file, xx, k, f"Mengunduh {filename}...")
    await xx.delete()
    dl = resayra.name
    if not msg:
...
async def imgscan(event):
    ...
    if not (
        ...
        await event.eor("`Balas ke Gambar saja...`")
        return
    ayra = await ok.download_media()
    xx = await event.eor(get_string("com_1"))
    image = cv2.imread(ayra)
    ...
    scann = f"Scanned {ayra.split('.')[0]}.pdf"
    im1.save(scann)
...
async def savepdf(event):
    ...
    if not (ok and (ok.media)):
        await eor(
            event,
            "`Balas ke Gambar/pdf yang ingin Anda gabungkan sebagai satu pdf..`",
        )
        return
    ayra = await ok.download_media()
    if ayra.endswith(("png", "jpg", "jpeg", "webp")):
        xx = await event.eor(get_string("com_1"))
        image = cv2.imread(ayra)
        original_image = image.copy()
        ratio = image.shape[0] / 500.0
        ...
        im1.save(a)
        await xx.edit(
            f"Selesai, Sekarang Balas Gambar/pdf lain jika selesai kemudian gunakan {HNDLR}pdsend untuk menggabungkan dan mengirim semua sebagai pdf",
        )
        os.remove("o.png")
    elif ayra.endswith(".pdf"):
        a = check_filename("pdf/scan.pdf")
        ...
        await event.eor("`Balas ke Gambar/pdf saja...`")
    os.remove(ayra)
...
async def sendpdf(event):
    if not os.path.exists("pdf/scan.pdf"):
        await eor(
            event,
            "pertama pilih halaman dengan membalas .pdsave yang ingin Anda buat file pdf multi halaman",
        )
        return
    msg = event.pattern_match.group(1).strip()
