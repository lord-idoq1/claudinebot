...
• `{i}hl <a link> <text-optional>`
    Sematkan tautan dengan spasi kosong sebagai pesan.
...
@ayra_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
    ...
    if txt:
        ...
        return await eor(
            event, f"`{HNDLR}tr LanguageCode` sebagai balasan pesan", time=5
        )
    lan = input or "id"
    try:
        tt = translate(text, lang_tgt=lan)
        output_str = f"**TERJEMAHAN** to {lan}\n{tt}"
...
@ayra_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ayra):
...
async def _(e):
    ...
    if not (reply and reply.media):
        return await e.eor("`Balas ke file gif atau audio saja.`")
    if "audio" in mediainfo(reply.media):
        ...
        await msg.edit("`Membuat catatan video...`")
        await bash(
        ...
        msg = await e.eor("**Membuat catatan video**")
        ...
        await e.eor("`Balas ke file gif atau audio saja.`")
...
async def lastname(steal):
    ...
    if not steal.is_reply and not mat:
        return await steal.eor("`Gunakan perintah ini dengan reply atau berikan Username/id...`")
    if mat:
        try:
