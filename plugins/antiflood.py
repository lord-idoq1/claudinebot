...
async def unmuting(e):
    ...
    user = int(ino[0])
    chat = int(ino[1])
    user_name = (await ayra_bot.get_entity(user)).first_name
    chat_title = (await ayra_bot.get_entity(chat)).title
    await ayra_bot.edit_permissions(chat, user, send_messages=True)
    await e.edit(
