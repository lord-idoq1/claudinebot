...
• `{i}mediainfo <reply to media>/<file path>/<url>`
   Untuk mendapatkan info tentangnya.

• `{i}rotate <degree/angle> <reply to media>`
   Putar video/foto/media apa saja..
   Catatan : untuk video sebaiknya sudut 90-an
"""
import os
import time
from datetime import datetime as dt

from Ayra.fns.misc import rotate_image
from Ayra.fns.tools import make_html_telegraph
...
async def mi(e):
    ...
    if r and r.media:
        ...
        extra = f"**[{xx}]({url})**\n\n"
        e = await e.eor(f"{extra}`Memuat Lebih Banyak...`", link_preview=False)
...
@ayra_cmd(pattern="rotate( (.*)|$)")
async def rotate_(ayra):
    match = ayra.pattern_match.group(1).strip()
    if not ayra.is_reply:
        return await ayra.eor("`Balas ke media...`")
    if match:
        try:
            match = int(match)
        except ValueError:
            match = None
    if not match:
        return await ayra.eor("`Berikan sudut yang valid untuk memutar media..`")
    ...
    if photo or reply.photo or reply.sticker:
        media = await ayra.client.download_media(photo or reply)
        ...
        return await msg.edit("`Media tidak didukung..\nBalas ke Foto/Video`")
    if os.path.exists(file):
        await ayra.client.send_file(
            ayra.chat_id, file=file, video_note=bool(reply.video_note), reply_to=reply.id
        )
    os.remove(media)
